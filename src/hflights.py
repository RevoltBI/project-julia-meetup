...
new_df = pd.DataFrame({"DateTime":[datetime.now().isoformat()]*len(task_names), "Language":["Python"]*len(task_names), "TaskNames":task_names, "TaskTypes":task_types, "TaskExecTimes":task_exec_times})
try:
    out_df = pd.concat([pd.read_csv("logs/log.csv"), new_df])
except Exception as e:
    out_df = new_df

out_df.to_csv("logs/log.csv")
